...
class TraceAspect {
    ...
    @After("methodLogger()")
    fun onMethodAfter(joinPoint: JoinPoint) {
        //在调用切点方法之后可以做些操作，就在这儿做
        //比如获取这个方法的类名或者调用方法前弹个提示信息，修改一些数据等等
        //我这儿就只输出一下方法的类名
        Log.e(TAG, "onMethodBefore: 类名：" + joinPoint.toString())
        //获取方法的类的对象，对其进行操作
        Log.e(TAG, "onMethodBefore: 是否是Activity？ " + (joinPoint.target is Activity))
        //还可以操作更多，自己慢慢探索
    }

    @Around("methodLogger()")
    @Throws(Throwable::class)
    fun onMethodAround(joinPoint: ProceedingJoinPoint): Any {
        //方法执行时，对其进行操作
        //方法执行开始，计时
